...
mod io {
    ...
    fn test_read_video() {
        ...
        let (green_history, frame_rate) = match cal::read_video(video_record, region_record) {
            Ok(res) => res,
    ...
    fn test_detect_peak() {
        let video_record = (START_FRAME, FRAME_NUM, &PATH.to_string());
        let region_record = (UPPER_LEFT_COORD, REGION_SHAPE);
        let green_history = cal::read_video(video_record, region_record).unwrap().0;

        let t0 = std::time::Instant::now();
        let peak = cal::detect_peak(green_history);
        println!("{:?}", std::time::Instant::now().duration_since(t0));
        println!("{}", peak.slice(s![180000..180100]));
    }

    #[test]
    fn test_read_excel_temp() {
        let excel_path = &"E:\\research\\CFD\\cfd_result.xlsx".to_owned();
        let temp_record = (1, 5, &vec![2, 3, 4, 5, 6, 7, 8], excel_path);
        let res = cal::read_excel_temp(temp_record).unwrap();
