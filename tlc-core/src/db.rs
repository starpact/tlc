...
use crate::{
    daq::{read_daq, DaqMeta, Interpolator, Thermocouple},
...
impl Database {
    ...
    pub fn save_root_dir(&self) -> anyhow::Result<&Path> {
        Ok(self
            .save_root_dir
            .as_ref()
    ...
    pub fn video_path(&self) -> anyhow::Result<&Path> {
        Ok(self
            .video_path
            .as_ref()
    ...
    pub fn video_nframes(&self) -> anyhow::Result<usize> {
        Ok(self.video_data()?.nframes())
    }

    pub fn video_frame_rate(&self) -> anyhow::Result<usize> {
        Ok(self.video_data()?.frame_rate())
    }

    pub fn video_shape(&self) -> anyhow::Result<(u32, u32)> {
        Ok(self.video_data()?.shape())
    }

    pub fn daq_path(&self) -> anyhow::Result<&Path> {
        Ok(self
            .daq_path
            .as_ref()
    ...
    pub fn start_frame(&self) -> anyhow::Result<usize> {
    ...
    pub fn start_row(&self) -> anyhow::Result<usize> {
    ...
    pub fn area(&self) -> anyhow::Result<(u32, u32, u32, u32)> {
    ...
    pub fn filter_method(&self) -> anyhow::Result<FilterMethod> {
        self.filter_method
    ...
    pub fn thermocouples(&self) -> anyhow::Result<&[Thermocouple]> {
        Ok(self
            .thermocouples
            .as_ref()
    ...
    #[instrument(level = "trace", skip(self), err)]
    pub fn set_thermocouples(&mut self, thermocouples: Box<[Thermocouple]>) -> anyhow::Result<()> {
    ...
    pub fn interp_method(&self) -> anyhow::Result<InterpMethod> {
        self.interp_method
    ...
    pub fn iter_method(&self) -> anyhow::Result<IterMethod> {
    ...
    pub fn physical_param(&self) -> anyhow::Result<PhysicalParam> {
        self.physical_param
    ...
    #[instrument(level = "trace", skip(self), err)]
    pub fn nu_plot(&self, trunc: Option<(f64, f64)>) -> anyhow::Result<String> {
        let name = self.name()?;
    ...
    pub fn video_data(&self) -> anyhow::Result<Arc<VideoData>> {
        read_video(self.video_path()?)
    }

    pub fn daq_data(&self) -> anyhow::Result<ArcArray2<f64>> {
        read_daq(self.daq_path()?)
    }
    ...
    pub fn nu2(&self) -> anyhow::Result<ArcArray2<f64>> {
        let video_data = self.video_data()?;
        let daq_data = self.daq_data()?;
        let start_index = self.start_index()?;
        let cal_num = eval_cal_num(video_data.nframes(), daq_data.nrows(), start_index);
